...
<!DOCTYPE html><html lang="en"><head>
  ...
  // Quick Guide Onboarding Modal Controller
  let currentGuideSlide = 1;
  const totalGuideSlides = 5;

  window.updateGuideSlide = function(step) {
    currentGuideSlide = step;
    const slides = document.querySelectorAll('.guide-slide-card');
    ...
    if (btnGuidePrev) btnGuidePrev.style.display = (step > 1) ? 'inline-flex' : 'none';
    if (btnGuideNext) btnGuideNext.textContent = (step === totalGuideSlides) ? 'Get Started 🚀' : 'Next →';
  };

  window.openQuickGuide = function() {
    window.updateGuideSlide(1);
  ...
  window.closeQuickGuide = function() {
    const modal = document.getElementById('quickGuideModal');
    if (modal) modal.style.display = 'none';
  };

  // Wire topbar & modal button listeners
  const btnQuickGuide = document.getElementById('btnQuickGuide');
  const btnCloseQuickGuide = document.getElementById('btnCloseQuickGuide');
  const btnGuidePrev = document.getElementById('btnGuidePrev');
  const btnGuideNext = document.getElementById('btnGuideNext');

  if (btnQuickGuide) btnQuickGuide.addEventListener('click', window.openQuickGuide);
  if (btnCloseQuickGuide) btnCloseQuickGuide.addEventListener('click', window.closeQuickGuide);

  if (btnGuideNext) {
    btnGuideNext.addEventListener('click', () => {
      if (currentGuideSlide < totalGuideSlides) {
        window.updateGuideSlide(currentGuideSlide + 1);
      } else {
        window.closeQuickGuide();
        localStorage.setItem('resuai_quick_guide_seen', 'true');
      }
    });
  }
  ...
  // Dot click navigation listeners
  document.querySelectorAll('.guide-dot').forEach(dot => {
    dot.addEventListener('click', (e) => {
      const dotNum = parseInt(e.target.getAttribute('data-dot'));
      if (dotNum) window.updateGuideSlide(dotNum);
    });
  });
